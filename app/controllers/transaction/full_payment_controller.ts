...
export default class FullPaymentController {
  async store({ request, params, response, session, auth, logger }: HttpContext) {
    ...
    const transaction = await order
      ...
      .query()
      .where('type', TransactionType.FULL_PAYMENT)
    ...
    if (isPending && !isExpired) {
      ...
      return response.redirect().toRoute('full_payment.show', { number: order.number })
    }
    ...
    const key = `payment_FP_${request.ip()}_${order.number}`

    const [error] = await paymentLimiter.penalize(key, async () => {
      const orderId = `FP_${order.number}_${DateTime.now().toFormat('yyyyMMddHHmmss')}`
    ...
    if (error) {
      logger.warn('Too many full payment attempts detected.')
      session.flash(
        'error',
