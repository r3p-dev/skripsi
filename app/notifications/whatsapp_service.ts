...
export default class WhatsappService {
  ...
  async sendPhoneChangeNotice(target: string, newPhone: string): Promise<void> {
    await this.#sendMessage(
      ...
      [
        'Umima.Clean menerima permintaan perubahan nomor telepon untuk akun Anda.',
        `Nomor baru yang diajukan: ${newPhone}`,
        'Tautan verifikasi dikirim ke nomor baru tersebut. Nomor Anda belum berubah sampai tautan itu dibuka.',
        'Jika Anda tidak mengajukan perubahan ini, atau nomor di atas keliru, abaikan tautannya dan segera hubungi kami.',
      ].join('\n\n')
    )
  }
